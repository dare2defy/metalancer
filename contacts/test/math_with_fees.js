...
contract("BPool", async accounts => {
  ...
  describe("With fees", () => {
    ...
    it("exitPool", async () => {
      ...
      // Balances of all tokens increase proportionally to the pool balance
      previousWethBalance = currentWethBalance;
      let balanceChange = Decimal(pAiAfterExitFee)
        .div(previousPoolBalance)
        .mul(previousWethBalance);
      currentWethBalance = currentWethBalance.sub(balanceChange);
      previousDaiBalance = currentDaiBalance;
      balanceChange = Decimal(pAiAfterExitFee)
        .div(previousPoolBalance)
        .mul(previousDaiBalance);
    ...
    it("joinswapExternAmountIn", async () => {
      ...
      const tAi =
        ...
        (currentWethBalance * (poolRatio ** (1 / wethNorm) - 1));
    ...
    it("joinswapPoolAmountOut", async () => {
      ...
      const tAi = await pool.joinswapPoolAmountOut.call(
        DAI,
        toWei(String(pAo)),
        MAX
    ...
    it("exitswapPoolAmountIn", async () => {
      ...
      // Update balance states
      previousPoolBalance = currentPoolBalance;
      currentPoolBalance = currentPoolBalance.sub(
        Decimal(pAi).mul(Decimal(1).sub(Decimal(exitFee)))
      );
      previousWethBalance = currentWethBalance;
      const mult =
        (1 - poolRatioAfterExitFee ** (1 / wethNorm)) *
        (1 - swapFee * (1 - wethNorm));
      currentWethBalance = currentWethBalance.sub(
        previousWethBalance.mul(Decimal(mult))
      );
    ...
    it("exitswapExternAmountOut", async () => {
      ...
      const pAi = await pool.exitswapExternAmountOut.call(
        DAI,
        toWei(String(tAo)),
        MAX
      );
      ...
      const expected = previousPoolBalance
        .sub(currentPoolBalance)
        .div(Decimal(1).sub(Decimal(exitFee)));
    ...
    it("pAo = joinswapExternAmountIn(joinswapPoolAmountOut(pAo))", async () => {
      ...
      const calculatedPAo = await pool.joinswapExternAmountIn.call(
        WETH,
        String(tAi),
        toWei("0")
      );
    ...
    it("tAi = joinswapPoolAmountOut(joinswapExternAmountIn(tAi))", async () => {
      ...
      const calculatedtAi = await pool.joinswapPoolAmountOut.call(
        DAI,
        String(pAo),
        MAX
      );
    ...
    it("pAi = exitswapExternAmountOut(exitswapPoolAmountIn(pAi))", async () => {
      ...
      const calculatedPAi = await pool.exitswapExternAmountOut.call(
        WETH,
        String(tAo),
        MAX
      );
    ...
    it("tAo = exitswapPoolAmountIn(exitswapExternAmountOut(tAo))", async () => {
      ...
      const calculatedtAo = await pool.exitswapPoolAmountIn.call(
        DAI,
        String(pAi),
        toWei("0")
      );
