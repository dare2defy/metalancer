...
contract("BPool", async accounts => {
  ...
  describe("Finalizing pool", () => {
    ...
    it("Only controller can setPublicSwap", async () => {
      ...
      assert(publicSwap);
      await truffleAssert.reverts(
        pool.setPublicSwap(true, { from: user1 }),
        "ERR_NOT_CONTROLLER"
      );
    });
  ...
  describe("User interactions", () => {
    ...
    it("swapExactAmountIn", async () => {
      ...
      const wethPriceFeeCheck =
        (10024.094194662908577 / 5 / (55 / 5)) * (1 / (1 - 0.003));
      assert.approximately(
        Number(fromWei(wethPrice)),
        Number(wethPriceFeeCheck),
        errorDelta
      );
