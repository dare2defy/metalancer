...
function calcOutGivenIn(
  ...
  const adjustedIn = Decimal(tokenAmountIn).times(
    Decimal(1).minus(Decimal(swapFee))
  );
  const y = Decimal(tokenBalanceIn).div(
    Decimal(tokenBalanceIn).plus(adjustedIn)
  );
