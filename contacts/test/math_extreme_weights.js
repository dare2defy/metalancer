...
contract("BPool", async accounts => {
  ...
  describe("Extreme weights", () => {
    ...
    it("joinPool", async () => {
      ...
      currentPoolBalance = Decimal(currentPoolBalance).add(
        Decimal(poolAmountOut)
      );

      // Balances of all tokens increase proportionally to the pool balance
      previousWethBalance = currentWethBalance;
      let balanceChange = Decimal(poolAmountOut)
        .div(previousPoolBalance)
        .mul(previousWethBalance);
      currentWethBalance = currentWethBalance.add(balanceChange);
      previousDaiBalance = currentDaiBalance;
      balanceChange = Decimal(poolAmountOut)
        .div(previousPoolBalance)
        .mul(previousDaiBalance);
    ...
    it("exitPool", async () => {
      ...
      const poolAmountInAfterExitFee = Decimal(poolAmountIn).mul(
        Decimal(1).sub(exitFee)
      );

      await pool.exitPool(toWei(String(poolAmountIn)), [
        toWei("0"),
        toWei("0")
      ]);
      ...
      // Balances of all tokens increase proportionally to the pool balance
      previousWethBalance = currentWethBalance;
      let balanceChange = poolAmountInAfterExitFee
        .div(previousPoolBalance)
        .mul(previousWethBalance);
      currentWethBalance = currentWethBalance.sub(balanceChange);
      previousDaiBalance = currentDaiBalance;
      balanceChange = poolAmountInAfterExitFee
        .div(previousPoolBalance)
        .mul(previousDaiBalance);
    ...
    it("joinswapExternAmountIn", async () => {
      ...
      const tokenAmountIn =
        (1 / (1 - swapFee * (1 - wethNorm))) *
        (currentWethBalance * (tokenRatio - 1));
      await pool.joinswapExternAmountIn(
        WETH,
        toWei(String(tokenAmountIn)),
        toWei("0")
      );
      // Update balance states
      previousWethBalance = currentWethBalance;
      ...
      previousPoolBalance = currentPoolBalance;
      currentPoolBalance = currentPoolBalance.mul(
        Decimal(tokenRatio).pow(wethNorm)
      ); // increase by 1.1**wethNorm
    ...
    it("joinswapPoolAmountOut", async () => {
      ...
      previousDaiBalance = currentDaiBalance;
      const numer = previousDaiBalance.mul(
        Decimal(poolRatio)
          .pow(Decimal(1).div(daiNorm))
          .sub(Decimal(1))
      );
      const denom = Decimal(1).sub(
        Decimal(swapFee).mul(Decimal(1).sub(daiNorm))
      );
    ...
    it("exitswapPoolAmountIn should revert", async () => {
      ...
      const poolAmountIn =
        currentPoolBalance * (1 - poolRatioAfterExitFee) * (1 / (1 - exitFee));
      await truffleAssert.reverts(
        pool.exitswapPoolAmountIn(
          WETH,
          toWei(String(poolAmountIn)),
          toWei("0")
        ),
        "ERR_MAX_OUT_RATIO"
      );
    ...
    it("exitswapExternAmountOut", async () => {
      ...
      const tokenAmountOut =
        ...
        (1 - tokenRatioBeforeSwapFee) *
        (1 - swapFee * (1 - daiNorm));
      await pool.exitswapExternAmountOut(
        DAI,
        toWei(String(tokenAmountOut)),
        MAX
      );
      // Update balance states
      previousDaiBalance = currentDaiBalance;
      ...
      previousPoolBalance = currentPoolBalance;
      const balanceChange = previousPoolBalance.mul(
        Decimal(1).sub(Decimal(poolRatioAfterExitFee))
      );
    ...
    it("poolAmountOut = joinswapExternAmountIn(joinswapPoolAmountOut(poolAmountOut))", async () => {
      ...
      const tokenAmountIn = await pool.joinswapPoolAmountOut.call(
        WETH,
        toWei(String(poolAmountOut)),
        MAX
      );
      const pAo = await pool.joinswapExternAmountIn.call(
        WETH,
    ...
    it("tokenAmountIn = joinswapPoolAmountOut(joinswapExternAmountIn(tokenAmountIn))", async () => {
      ...
      const poolAmountOut = await pool.joinswapExternAmountIn.call(
        DAI,
        toWei(tokenAmountIn),
        toWei("0")
      );
      const calculatedtokenAmountIn = await pool.joinswapPoolAmountOut.call(
        DAI,
        String(poolAmountOut),
        MAX
      );
    ...
    it("poolAmountIn = exitswapExternAmountOut(exitswapPoolAmountIn(poolAmountIn))", async () => {
      ...
      const tokenAmountOut = await pool.exitswapPoolAmountIn.call(
        WETH,
        toWei(String(poolAmountIn)),
        toWei("0")
      );
      const calculatedpoolAmountIn = await pool.exitswapExternAmountOut.call(
        WETH,
        String(tokenAmountOut),
        MAX
      );
    ...
    it("tokenAmountOut = exitswapPoolAmountIn(exitswapExternAmountOut(tokenAmountOut))", async () => {
      ...
      const poolAmountIn = await pool.exitswapExternAmountOut.call(
        DAI,
        toWei(String(tokenAmountOut)),
        MAX
      );
      const tAo = await pool.exitswapPoolAmountIn.call(
        DAI,
        String(poolAmountIn),
        toWei("0")
      );
