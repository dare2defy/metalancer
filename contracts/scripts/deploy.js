...
// Deployer account: 0x802f53aFD50ebB03d17CAE8104779965FDe00034
...
// Minted TOK1 for 0x802f53aFD50ebB03d17CAE8104779965FDe00034, balance: 10000000000000000000000
// Minted TOK2 for 0x802f53aFD50ebB03d17CAE8104779965FDe00034, balance: 20000000000000000000000
// Minted TOK3 for 0x802f53aFD50ebB03d17CAE8104779965FDe00034, balance: 30000000000000000000000
...
async function main() {
  ...
  let tokensBalances = ["10000", "20000", "30000"];
  for (let i = 0; i < 2; i++) {
    for (let t = 0; t < 3; t++) {
      const mintTx = await tokens[t].mint(
        accounts[i],
        web3.utils.toWei(tokensBalances[t])
      );
      await mintTx.wait();
      console.log(
        "Minted " +
  ...
  const txApprove1 = await tokens[0].approve(
    pool_address,
  ...
  const txApprove2 = await tokens[1].approve(
    pool_address,
  ...
  const txApprove3 = await tokens[2].approve(
    pool_address,
  ...
  const bind1Tx = await bpool.bind(
    tokens[0].address,
    web3.utils.toWei("10000"),
    web3.utils.toWei("5")
  );
  ...
  const bind2Tx = await bpool.bind(
    tokens[1].address,
    web3.utils.toWei("20000"),
    web3.utils.toWei("5")
  );
  ...
  const bind3Tx = await bpool.bind(
    tokens[2].address,
    web3.utils.toWei("30000"),
    web3.utils.toWei("5")
  );
